...
class RabbitMQClient {
  ...
  async SetDefinitions() {
    try {
      ...
      try {
        await this.highThroughputChannel.assertExchange(
          this.definitions.exchange.general,
          "direct",
          { durable: true },
        );
      ...
      try {
        await this.highThroughputChannel.assertExchange(
          this.definitions.exchange.crawler,
          "direct",
          { durable: true },
        );
      ...
      await this.eventsChannel.bindQueue(
        this.definitions.queues.se_db_request_queue,
        this.definitions.exchange.general,
        this.definitions.routing_keys.se_db_request,
      );
      await this.eventsChannel.bindQueue(
        this.definitions.queues.es_db_check_queue,
        this.definitions.exchange.general,
        this.definitions.routing_keys.es_db_check,
      );

      await this.eventsChannel.bindQueue(
        this.definitions.queues.cr_db_getlen_queue,
        this.definitions.exchange.crawler,
        this.definitions.routing_keys.cr_db_getlen,
      );
      await this.eventsChannel.bindQueue(
        this.definitions.queues.cr_db_indexing_queue,
        this.definitions.exchange.crawler,
        this.definitions.routing_keys.cr_db_indexing,
      );
      await this.eventsChannel.bindQueue(
        this.definitions.queues.cr_db_enqueue_queue,
        this.definitions.exchange.crawler,
        this.definitions.routing_keys.cr_db_enqueue,
      );
      await this.eventsChannel.bindQueue(
        this.definitions.queues.cr_db_dequeue_queue,
        this.definitions.exchange.crawler,
        this.definitions.routing_keys.cr_db_dequeue,
      );
    } catch (err) {
      const error = err as Error;
      console.error("ERROR: Something went wrong while creating channels.");
      console.error(err);
      throw new Error(error.message);
    }
  }

  SearchEngineHandler(pool: mysql.Pool) {
    /*
		...
		*/
    console.log(
      `database consume queue: ${this.definitions.queues.se_db_request_queue}`,
    );
    this.eventsChannel!.consume(
      ...
      async (data) => {
        ...
        try {
          ...
          console.log({
            searchEngineMessage: data.content.toString(),
            BuffLen: Math.trunc(blen.byteLength / MSS) + 1,
          });
        } catch (err) {
          const error = err as Error;
    ...
    this.eventsChannel!.consume(
      ...
      async (msg: amqp.ConsumeMessage | null) => {
        ...
        try {
          ...
          console.log(
            "dbInterface TEST: QUEUE='%s' LENGTH ",
            hostname,
            queueLen,
          );
...
const yamlfile = fs.readFileSync(
  path.join(import.meta.dirname, "../../../rabbitmq.yml"),
  "utf8",
);
...
const expressDef: DatabaseServiceDefinition = {
  ...
  queues: {
    ...(doc.queues["express_server_queues"] as any),
    ...(doc.queues["search_engine_queues"] as any),
    ...(doc.queues["crawler_queues"] as any),
  },
