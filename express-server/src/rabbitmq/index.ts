...
class RabbitMQClient {
  ...
  async SetDefinitions() {
    try {
      ...
      await this.eventsChannel.assertQueue(
        this.definitions.queues.es_cr_request_queue,
        { exclusive: false, durable: true },
      );
      await this.eventsChannel.assertQueue(
      ...
      await this.publishChannel.bindQueue(
        this.definitions.queues.es_cr_request_queue,
        this.definitions.exchange.general,
        this.definitions.routing_keys.es_cr_request,
      );
      await this.publishChannel.bindQueue(
        this.definitions.queues.es_se_query_queue,
        this.definitions.exchange.general,
  ...
  async Crawl(
    ...
    try {
      const success = this.publishChannel!.publish(
        this.definitions.exchange.general,
        this.definitions.routing_keys.es_cr_request,
        websites,
        {
  ...
  /*
	  ...
	  */
  async DBCheckHandler(encoded_list: Uint8Array): Promise<{
    unindexed: Array<string>;
  }> {
    this.publishChannel!.publish(
      this.definitions.exchange.general,
      this.definitions.routing_keys.es_db_check,
      Buffer.from(encoded_list),
      {
...
const yamlfile = fs.readFileSync(
  path.resolve(import.meta.dirname, "../../../rabbitmq.yml"),
  "utf8",
);
